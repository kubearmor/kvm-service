...
func (dm *KVMS) GenerateExternalWorkloadIdentity(label string) uint16 {
	for {
		...
		if resp := K8s.WatchK8sExternalWorkloadSecurityPolicies(); resp != nil {
			...
			for {
				...
				if event.Type == "ADDED" {
                    ...
                    identity := dm.GenerateExternalWorkloadIdentity(secPolicy.Metadata["labels"])
                    //gs.GenerateEWInstallationScript(secPolicy.Metadata["labels"], identity)
